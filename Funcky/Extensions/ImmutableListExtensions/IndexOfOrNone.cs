...
public static partial class ImmutableListExtensions
{
    ...
    /// <returns>The zero-based index of the first occurrence of <paramref name="item" /> within the range of elements in the <see cref="IImmutableList{T}" /> that starts at <paramref name="index" /> and contains <paramref name="count" /> number of elements if found; otherwise <see cref="Option{T}.None" />.</returns>
    public static Option<int> IndexOfOrNone<TItem>(this IImmutableList<TItem> list, TItem item, int index, int count, IEqualityComparer<TItem>? equalityComparer)
        => MapNotFoundToNone(list.IndexOf(item, index, count, equalityComparer));
    ...
    /// <param name="item">The object to locate in the <see cref="T:System.Collections.Immutable.IImmutableList`1" />. This value can be null for reference types.</param>
    /// <param name="index">The zero-based starting indexes of the search. 0 (zero) is valid in an empty list.</param>
    /// <returns>The zero-based index of the first occurrence of <paramref name="item" /> within the range of elements in the <see cref="IImmutableList{T}" /> that starts at <paramref name="index" /> if found; otherwise <see cref="Option{T}.None" />.</returns>
    public static Option<int> IndexOfOrNone<TItem>(this IImmutableList<TItem> list, TItem item, int index)
        => MapNotFoundToNone(list.IndexOf(item, index));
    ...
    /// <param name="count">The number of elements in the section to search.</param>
    /// <returns>The zero-based index of the first occurrence of <paramref name="item" /> within the range of elements in the <see cref="IImmutableList{T}" /> that starts at <paramref name="index" /> and contains <paramref name="count" /> number of elements if found; otherwise <see cref="Option{T}.None" />.</returns>
    public static Option<int> IndexOfOrNone<TItem>(this IImmutableList<TItem> list, TItem item, int index, int count)
        => MapNotFoundToNone(list.IndexOf(item, index, count));
}
